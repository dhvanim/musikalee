...
class Search(unittest.TestCase):
    ...
    def test_spotify_search(self):
        with mock.patch(
            "spotify_music.spotify_get_access_token", GetAccessToken().mock_access_token
...
class GetTrending(unittest.TestCase):
                ...
                "items": [
                    {
                        "tracks": {
                            ...
                            37i9dQZEVXbLRQDuF5jeBp?si=DevSoVAKT5WiU7YfwFFc7A"""
                        }
                    }
                ]
            }
    ...
    def test_trending_success(self):
        with mock.patch(
            "spotify_music.spotify_search", self.mock_search_success
        ), mock.patch(
            "spotify_music.spotify_get_access_token", GetAccessToken().mock_access_token
        ), mock.patch(
            "spotify_music.spotmusic_api.trendcall", self.mock_request_track_success
        ):
    ...
    def test_trending_search_failure(self):
        with mock.patch(
            "spotify_music.spotify_search", MockedSearch().failure
        ), mock.patch(
            "spotify_music.spotify_get_access_token", GetAccessToken().mock_access_token
        ), mock.patch(
            "spotify_music.spotmusic_api.trendcall", self.mock_request_track_success
    ...
    def test_trending_tracks_failure(self):
        with mock.patch(
            ...
            "spotify_music.spotmusic_api.trendcall", self.mock_request_track_failure
        ):
            result = spotify_music.spotify_get_trending()
...
class GetRecommended(unittest.TestCase):
    ...
    def mock_request_rec(self, artists, token):
        if not artists or not token:
            return None

        return {
            "tracks": [{"name": "Self Control", "artists": [{"name": "Frank Ocean"}]}]
        }

    def test_recommended_success(self):
        with mock.patch(
            "spotify_music.spotify_get_access_token", GetAccessToken().mock_access_token
        ), mock.patch("spotify_music.spotmusic_api.getrecocall", self.mock_request_rec):
            results = spotify_music.spotify_get_recommended(
                ["1anyVhU62p31KFi8MEzkbf", "2P5sC9cVZDToPxyomzF1UH"]
            )
...
class SearchPlaylist(unittest.TestCase):
    ...
    def test_playlist_success(self):
        with mock.patch(
            ...
            "spotify_music.spotmusic_api.search_playlist",
            self.mock_request_playlist_success,
        ):
            results = spotify_music.spotify_search_playlist(
                "https://open.spotify.com/playlist/0Ix3TaQtxZb7cZNpFV1YKi?si=kUhWWU0lQ4SVUj8NYyYWxQ"
            )
    ...
    def test_playlist_search_failure(self):
        ...
        with mock.patch(
            ...
            "spotify_music.spotmusic_api.search_playlist",
            self.mock_request_playlist_failure,
        ):
            results = spotify_music.spotify_search_playlist(
                "https://open.spotify.com/playlist/3544443543"
            )
...
class MockedSearch:
    ...
    def empty(self, query, query_type):
        ...
        search_mock.status_code = 200
        title = query_type + "s"
        search_mock.json.return_value = {title: {"total": 0}}
        return search_mock
